...
export default function RoomPage({ params }: RoomPageProps) {
  ...
  const onChannelReady = useCallback(() => {
    setSnackbar({ message: "Peer connected — ready to send" });
  }, []);

  const onChannelMessage = useCallback(
    (event: MessageEvent, peerId: string) => handleIncomingMessage(event, peerId),
    [handleIncomingMessage]
  );

  const { connected, peersRef } = useWebRTC({
    roomId,
    selfId,
    onChannelReady,
    onChannelMessage,
  ...
  useEffect(() => {
    setSelfId(selfId);
    return () => reset();
  }, [selfId, setSelfId, reset]);

  const connectedPeerIds = Object.values(peers)
  ...
  const handleFiles = useCallback(
    (files: File[]) => {
      if (connectedPeerIds.length === 0) {
        setSnackbar({ message: "No peer connected yet", tone: "error" });
        return;
      }
      ...
      connectedPeerIds.forEach((peerId) => {
        ...
        if (channel && channel.readyState === "open") {
          files.forEach((file) => sendFile(channel, file, peerId));
        }
      });
    },
    ...
    <main className="min-h-screen flex flex-col">
      <header className="flex items-center justify-between px-5 sm:px-8 py-5 border-b border-outline-variant/40">
        <div className="flex items-center gap-3">
          <a href="/" className="font-display text-lg font-semibold text-on-surface">
            Signal
          </a>
          <span className="font-mono text-xs text-on-surface-variant px-2 py-1 rounded-m3-xs bg-surface-container-high">
            {roomId}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-on-surface-variant hover:text-on-surface transition-colors p-2 rounded-m3-full hover:bg-surface-container-high"
          >
            {muted ? <IconMuted /> : <IconSound />}
          </button>
          <ThemePicker />
          <M3Button variant="tonal" onClick={copyLink} className="hidden sm:inline-flex">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-[280px_1fr] gap-6 px-5 sm:px-8 py-6 max-w-6xl w-full mx-auto">
        <aside className="md:border-r md:border-outline-variant/40 md:pr-6">
          <h2 className="font-display text-sm font-medium text-on-surface-variant uppercase tracking-wide mb-3">
            Peers
          </h2>
          <PeerList />
          ...
          <M3Button
            variant="text"
            onClick={copyLink}
            className="sm:hidden w-full mt-4 justify-center"
          >
